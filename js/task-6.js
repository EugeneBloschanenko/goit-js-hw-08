...
const numberOfElements = document.querySelector('.inputNumber');
const buttonCreate = document.querySelector('.buttonCreate');
const buttonDestroy = document.querySelector('.buttonDestroy');
const creativeSpace = document.querySelector('#boxes');

let firstBox =`<div style="width: 30px; height: 30px; background-color:${getRandomHexColor()};"></div>`;
...
function createBoxes () {
  ...
  const numberOfBoxes = parseInt(numberOfElements.value);

  if (numberOfBoxes > 0 && numberOfBoxes <= 100) {
    for (let i = 0; i < numberOfBoxes; i +=1) {
      boxesStyle += `<div class="boxStyle" style="width: ${startBoxWidth}px; height: ${startBoxHeight}px; background-color:${getRandomHexColor()};"></div>`; 
      startBoxWidth += step;
      startBoxHeight += step;
    };

    creativeSpace.insertAdjacentHTML('beforeend', boxesStyle);
  };
  };
